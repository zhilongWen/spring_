      ...
      <el-form label-width="80px">
        <el-form-item label="作业列表">
          <el-select v-model="jobType">
            <el-option :key="'SimpleJob'" :value="'简单任务(测试)'" />
            <el-option :key="'MySQLJob'" :value="'MySQL数据监控任务'" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="jobType === 'MySQL数据监控任务'" label="数据库名">
          <el-select v-model="databaseName">
            <el-option :key="'gmall'" :value="'yuntai'" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="jobType === 'MySQL数据监控任务'" label="表名">
          <el-select v-model="tableName">
            <el-option :key="'table'" :value="'yuntai_zuoyuan'" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="jobType === 'MySQL数据监控任务'" label="字段名">
          <el-select v-model="fieldName">
            <el-option :key="'age'" :value="'age'" />
            <el-option :key="'height'" :value="'height'" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="jobType === 'MySQL数据监控任务'" label="任务">
          <el-select v-model="MySQLJobType">
            <el-option :key="'nullRate'" :value="'空值率'" />
          </el-select>
        </el-form-item>
        <el-form-item label="任务类型">
          <el-select v-model="cronJob">
            <el-option :key="'SimpleTypeJob'" :value="'周期执行任务'" />
            <el-option :key="'CronTypeJob'" :value="'Cron任务'" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="cronJob === '周期执行任务'" label="执行周期">
          <el-select v-model="repeatTime">
            <el-option :key="'TenSeconds'" :value="'每10秒钟执行一次'" />
            <el-option :key="'OneMinute'" :value="'每分钟执行一次'" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="cronJob === 'Cron任务'" label="Cron表达式">
